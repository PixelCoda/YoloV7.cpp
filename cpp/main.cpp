...
void YOLOV7::detect(Mat& frame)
{
	...
	std::ifstream myfile("../../data/custom_weight.names");
	...
	for (size_t i = 0; i < indices.size(); ++i)
	{
		...
		sprintf(buf, "  {\"class_id\":%d, \"name\":\"%s\", \"coordinates\":{\"x\":%d, \"y\":%d, \"width\":%d, \"height\":%d}, \"confidence\":%f}",
					classIds[idx], myLines[classIds[idx]].c_str(), box.x, box.y, box.width, box.height, confidences[idx]);
